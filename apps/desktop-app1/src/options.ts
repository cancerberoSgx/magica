...
type RP = 'imageRotate' | 'imageSize' | 'command' | 'fields'|'theme'
...
export class Options extends StateComponent<CommonProps> {
  ...
  relevantProperties: RP[] = ['imageRotate', 'imageSize', 'command', 'fields', 'theme']
 ...
 protected handleFieldChange(id: string, value: string): any {
  ...
  protected handleCommandChange(value: string) {
    this.setState({ command: value, fields: this.state.commands.find(c => c.name === value)!.fields })
  }
...
<label>Rotate:<br/><input id="rotate" type="range" value="${this.state.imageRotate}" onchange="app1.handleRotate(this.valueAsNumber)" min="0" max="360"></label><br/><span id="rotateLabel">${this.state.imageRotate}</span> degrees<br/>
